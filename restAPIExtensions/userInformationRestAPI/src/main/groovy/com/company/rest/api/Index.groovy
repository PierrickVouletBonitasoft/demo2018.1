...
class Index implements RestApiController {
    ...
    RestApiResponse doHandle(HttpServletRequest request, RestApiResponseBuilder responseBuilder, RestAPIContext context) {
		...
		List<Group> groups = context.apiClient.identityAPI.getGroups(p*c, c, GroupCriterion.LABEL_ASC);
		...
		// Iterate over each user
		for (group in groups) {
			def groupInfo = [group: group.label];
			groupInformation << groupInfo
			
		}
		
		// Prepare the result
		def result = [p: p, c: c, groupInformation: groupInformation]
		
		int startIndex = p*c
		int endIndex = p*c + groupInformation.size() - 1
		
		// Send the result as a JSON representation
		return buildPagedResponse(responseBuilder, new JsonBuilder(result).toPrettyString(), startIndex, endIndex, context.apiClient.identityAPI.numberOfGroups)
